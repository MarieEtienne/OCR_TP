...
Toutes les espèces sont présentes, avec des fréquences d'occurrence allant de `r min(representation)` % à `r max(representation)` %. En revanche, `r sum(!valid_sites)` sites ne comptent aucune observation : ils ont été supprimés, ce qui fait passer le jeu de données de `r nrow(data_var)` à `r nrow(data_var_clean)` lignes.

Les variables explicatives se répartissent en trois types :

- **numériques** : SR, NR, OR ;
- **ordinales** : RR, BR ;
- **catégorielles** : TR, VR, SUR1, SUR2, SUR3, UR, FR, MR, CR.
