...
```{r}
#| label: fig-chp1-diversité
#| fig-cap: "diversité alpha"
#| echo: false
#| warning: false
#| message: false
ggplot(diversite_alpha_resume,
  ...
  scale_x_discrete(
    labels = c("Brown.frogs" = "Grenouille verte"
      "Common.toad" = "Crapaud commun",
      "Tree.frogs" = "Rainette verte",
      "Green.frogs" = "Grenouille verte",
      "Rana_temporaria" = "Grenouille rousse"
    )
  ) +
  labs(
    x = "Espèces",
...
Pour réalisé des modeles, il est necessaire de preparer notre jeu de de donnee correctement. Pour cela il faut premierermenet verifier que toutes nos especes sont bien representer dans l'esneble du jeu de donne et qu'il n'y ai pas une sous-representation. Cela doit etre egalement réalisé pour les sites étudié.

```{r chapitre3-dataprep-b, echo = FALSE, warning = FALSE, message = FALSE}
cat("Représentation du jeu de données (en %):")
...
cat("Dans le jeu de données il y a",sum(!valid_sites), "avec '0' observation passant alors de", nrow(data_var), "lignes à", nrow(data_var_clean), "\n")
```

Ces restultats montrent que: tout nos especes sont bien respresenter et que 6 sites on etait supprimer par sous-represnetaiton.
...
#| tbl-subcap: "Sélection basée sur un seuil de corrélation |r| > 0.70 (test de Spearman via `cor.test`). Les p-values ont été vérifiées et ajustées."
#| echo: false
tab <- data.frame(
  `Paire` = c("**RR** (distance aux routes) et **BR** (distance aux bâtiments)", "**UR** (utilisation du réservoir) et **FR** (présence de pêche)"),
  `r` = c("$0.80$", "$0.84$"),
  `p` = c("$100$", "$100$"), #< METTRE BONNES VALEURS ICI @P !>
...
#| tbl-subcap: "Association mesurée par le V de Cramér (issu du test du $\\chi^2$). Sélection basée sur un seuil $V > 0.60$ pour réduire la redondance entre variables catégorielles."
#| echo: false
tab <- data.frame(
...
$$
\text{logit}(p_i) = \beta_0 + \beta_1 FR_i + \beta_2 RR_i
$$
où :

- $p_i$ correspond à la probabilité que l'individu $i$ soit présent ;
- $\beta_0$ est l'ordonnée à l'origine du modèle ;
- $\beta_1$ représente l'effet de la variable $FR$ sur le log-odds de présence ;
- $\beta_2$ représente l'effet de la variable $RR$ sur le log-odds de présence ;
- $FR_i$ et $RR_i$ sont les valeurs observées de ces deux variables pour l'individu $i$.
...
geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2, fill = "#2b5c8f", color = NA) +
  annotate("text", x = Inf, y = Inf, label = "p = 0.00184 (**)", hjust = 1.1, vjust = 1.5, size = 3, fontface = "italic") +
  theme_minimal() +
...
p_rr <- ggplot(grid_pred_rr, aes(x = RR, y = Probability)) +
  geom_line(linewidth = 1.1, color = "#2b8f5c") +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2, fill = "#2b8f5c", color = NA) +
  annotate("text", x = Inf, y = Inf, label = "p = 0.231 (NS)", hjust = 1.1, vjust = 1.5, size = 3, fontface = "italic") +
  theme_minimal() +
  labs(title = "Effet des routes (RR)", x = "Distance / Intensité des routes (RR)", y = "Probabilité de présence")
```
...
Le modèle logistique ajusté s'écrit :
  ...
  $$
  \text{logit}(p_i) = -0.39678 + 0.12983 \times FR_i + 0.02676 \times RR_i
$$
...
En conclusion, le modèle ne met pas en évidence d'effet significatif de la distance aux routes (RR) sur la probabilité de présence des amphibiens ($p = 0.231$) (@fig-effet-global-peche-route & @tbl-glm-resultats). En revanche, l'intensité de la pêche (FR) exerce une influence significative sur leur présence ($p = 0.00184$) (@fig-effet-global-peche-route & @tbl-glm-resultats), suggérant que cette pression anthropique locale joue un rôle important dans la distribution globale des amphibiens. 
