...
Les analyses multivariées permettent d'analyser simultanément de multiples variables afin de découvrir des modèles, des structures et des relations complexes qui ne peuvent être mis en évidence par l'analyse isolée de chaque variable (@amphi_wikipedia_multivariate_statistics).
...
anova_cca <- anova(cca_model, by = 'term')
```
Le modèle explique `r round((cca_model$CCA$tot.chi / cca_model$tot.chi) * 100, 1)`% de la variance totale. Nos variables explicatives expliquent `r round((cca_model$CCA$eig[1] / cca_model$CCA$tot.chi) * 100,1)`% de la variance sur le premier axe, et `r round((cca_model$CCA$eig[2] / cca_model$CCA$tot.chi) * 100, 1)`% sur le deuxième.

Pour poursuivre cette analyse, un test ANOVA par permutation (999) a été réalisé afin de tester la significativité de l'inertie contrainte par le modèle. Le test révèle que nos variables explicatives influencent significativement la présence-absence de nos  amphibiens (F = `r round(anova_cca$F[1], 2)`, p = `r anova_cca$"Pr(>F)"[1]`). Au moment d'évaluer l'effet des variables par terme nous nos retrouvons avec $TR$, et $OR$.
