...
library(patchwork)
library(broom)
library(modelsummary) # modelsummary() est utilisé dans le chapitre 4
...
Amphibien très aquatique, de 6 à 12 cm, au dos vert à brun marqué de taches
sombres, souvent avec une ligne claire dorsale. On la trouve toute l'année
dans ou près des plans d'eau, et les mâles chantent au printemps et en été.
[@amphi_wikipedia_multivariate_statistics]
...
=======
  <div>
    ...
    <p class="credit">
      Par <a href="https://commons.wikimedia.org/wiki/User:LivingShadow">LivingShadow</a> —
      Travail personnel, <a href="http://creativecommons.org/publicdomain/zero/1.0/deed.en">CC0</a>,
      <a href="https://fr.wikipedia.org/wiki/Grenouille_rousse">Lien</a>
    </p>
  </div>

La grenouille rousse est une espèce commune des milieux humides,
des prairies et des forêts. Son corps est trapu et sa couleur varie
du brun clair au brun foncé. Elle mesure généralement entre 5 et 10 cm.
[@amphi_brunes]
...
=======
  <div>
    ...
    <p class="credit">
      Par <a href="https://commons.wikimedia.org/w/index.php?title=User:Heston_Benstead&action=edit&redlink=1">Heston_Benstead</a> —
      Travail personnel, <a href="http://creativecommons.org/publicdomain/zero/1.0/deed.en">CC0</a>,
      <a href="https://fr.wikipedia.org/wiki/Rana_dalmatina">Lien</a>
    </p>
  </div>

Plus élancée que la grenouille rousse, la grenouille agile possède
de longues pattes arrière qui lui permettent de réaliser de grands sauts.
Elle fréquente surtout les bois et les lisières forestières.
...
Le crapaud commun est un amphibien robuste et trapu, se nourrit principalement
d'escargots, de vers de terre ou autres petits insectes. Il vit dans les zones
humides (étangs, mares, sous-bois).

</div>
=======

  <!-- ===== FICHE : CRAPAUD COMMUN ===== -->
<div class="esp-fiche" markdown="1" id="common-toad">
<div style="display:flex; gap:20px; align-items:flex-start;">
  <div>
    <img src="images/crapaud_commun.jpg" alt="Crapaud commun">
    <p class="credit">
      Par <a href="/https://commons.wikimedia.org/wiki/User:Aka">Aka</a> —
      Travail personnel, <a href="http://creativecommons.org/publicdomain/zero/1.0/deed.en">CC0</a>,
      <a href="https://fr.wikipedia.org/wiki/Crapaud_commun">Lien</a>
    </p>
  </div>
  <div>
      <h3>Crapaud commun <em>(Bufo bufo)</em></h3>
...
Les Bombina spp., communément appelés les crapauds sonneurs,
...
uniques qui les distinguent à la fois des vrais crapauds et des grenouilles.
...
Hyla arborea, communément appelée la Rainette verte,
...
Elle possède un mode de vie presque exclusivement arboricole.
...
de richesse spécifique [@xia2023alpha]. Afin de comparer les communautés d'amphibiens
...
Malgré ces différences quantitatives, le classement relatif des espèces reste globalement similaire entre les deux autoroutes. Cette tendance suggère que les communautés présentes le long de l'A1 et de la S52 partagent une composition spécifique comparable, mais que les populations semblent plus abondantes sur la S52. Plusieurs facteurs peuvent expliquer ce résultat, notamment une qualité d'habitat plus favorable, une meilleure connectivité écologique [@clauzel2024connectivity] ou encore des différences dans les conditions d'échantillonnage. Des analyses statistiques complémentaires seraient toutefois nécessaires afin de déterminer si ces écarts sont significatifs.

### Diversité bêta : similitude des communautés entre autoroutes
Si la diversité alpha permet de caractériser les communautés localement, elle ne renseigne pas sur le degré de similitude entre les assemblages observés. Afin d'évaluer les différences de composition spécifique entre les relevés effectués sur les deux autoroutes, une analyse de la diversité bêta [@chen2010beta] a été réalisée à partir des distances de Jaccard calculées sur les données de présence-absence (@fig-chp1-diversité_beta).
...
data_amphi <- data %>% dplyr::select((last_col() - 6):last_col())
data_var   <- data %>% dplyr::select(2:(last_col() - 7))
...
Nous avions déjà explorer la diversité alpha et beta de nos amis amphibiens, mais pour réaliser des modèles il est nécessaire de préparer correctement le jeu de données. Il faut alors d'abord vérifier que toutes les espèces sont bien représentées, et puis écarter les sites où aucune espèce n'a été observée.

Selon les analyses réalisés, toutes les espèces sont présentes, avec des fréquences d'occurrence allant de `r min(representation)` % à `r max(representation)` % comme indiqué dans @tbl-chp3-representation. En revanche, `r sum(!valid_sites)` sites ne comptent aucune observation : ils ont été donc supprimés, ce qui fait passer le jeu de données de `r nrow(data_var)` à `r nrow(data_var_clean)` échantillons.
...
### Corrélations
Nous avons ensuite vérifié les corrélations entre variables explicatives afin d'éviter la multicolinéarité dans les modèles : corrélation de Spearman pour les variables numériques et ordinales, et V de Cramér pour les variables catégorielles. Dans les deux cas, le seuil de redondance retenu est de $0,70$.
...
data_num_ord <- data_var_clean %>% dplyr::select(SR, NR, OR, RR, BR)
## Variables catégorielles
data_cat <- data_var_clean %>%
   dplyr::select(TR, VR, SUR1, SUR2, SUR3, UR, FR, MR, CR, Motorway) %>% mutate(across(everything(), as.factor))
```
...
La matrice de corrélation de Spearman (@fig-chp3-cor-spearman) met en évidence une seule paire dépassant le seuil $|r| > 0,70$ : RR–BR. Une seule de ces deux variables a donc été conservée (@tbl-chp3-cor-num). Toutes les autres corrélations restent inférieures à $0,70$ en valeur absolue.
...
La matrice des V de Cramér (@fig-chp3-cor-cramer) révèle deux associations supérieures à $0,70$ : MR–CR. Les variables retenues sont indiquées dans le @tbl-chp3-cor-cat.
...
#| tbl-cap: "Variables associées (V de Cramér) et sélection des variables catégorielles. Association mesurée par le V de Cramér (issu du test du $\\chi^2$). Sélection basée sur un seuil $V > 0.70$ pour réduire la redondance entre variables catégorielles."
#| echo: false
tab <- data.frame(
  `Paire` = c("**MR** (état de maintenance) et **CR** (type de rive)"),
  `r` = c("$0.71$"),
  `Lien écologique` = c("Les rives artificialisées (béton) vont souvent de pair avec des milieux plus modifiés ou urbanisés, où l'entretien et les dépôts de déchets posent problème."),
  `Conservée` = c("**CR**"),
  `Éliminée` = c("**MR**"),
  check.names = FALSE, stringsAsFactors = FALSE
)
...
vars_cat_drop <- c("MR")   # catégorielles

# Suppression dans les data frames
data_num_ord_clean <- data_num_ord[, !colnames(data_num_ord) %in% vars_num_drop,
                                   drop = FALSE]
data_cat_clean <- data_cat[, !colnames(data_cat) %in% vars_cat_drop,
                           drop = FALSE]

# Assemblage (les variables catégorielles restent des facteurs)
data_var_clean <- cbind(data_num_ord_clean, data_cat_clean)
```

## Création des modèles pour étudier le jeu de donnée
### Analyse multivarié à deux tableaux: structuration de la composition de la communauté d'amphibiens
Avec ce même jeu de données, on se demande **quelles variables structurent la composition et la diversité globale de la communauté d'amphibiens**. Pour cela, une approche appropriée est l'analyse multivariée.💡^[Les analyses multivariées permettent d'analyser simultanément de multiples variables afin de découvrir des modèles, des structures et des relations complexes qui ne peuvent être mis en évidence par l'analyse isolée de chaque variable (@amphi_wikipedia_multivariate_statistics).]

Dans ce cas, ayant un jeu de données avec des informations sur la présence/absence des amphibiens et des variables catégorielles et numériques sur le contexte environnemental, une analyse canonique des correspondances (CCA) a été retenue 💡^[Cette méthode permet de contraindre l'ordination de la communauté d'amphibiens par un ensemble de variables explicatives, et ainsi d'identifier lesquelles structurent le plus la composition observée.].

#### Construction du modèle
```{r cca-graph, echo = FALSE, warning = FALSE, message = FALSE}
set.seed(1234)

# vegan::cca ne tolère pas les NA : on garde les sites complets
complets <- complete.cases(data_var_clean)
data_amphi_cca <- data_amphi_clean[complets, ]
data_var_cca   <- droplevels(data_var_clean[complets, ])

# 1. Analyse canonique des correspondances (CCA) avec les variables retenues
cca_model <- cca(data_amphi_cca ~ ., data = data_var_cca)

# 2. Test global de significativité du modèle
set.seed(1234)
anova_cca <- anova(cca_model)

# 3. Test par axe : quels axes contiennent une information significative ?
set.seed(1234)
anova_axis_cca <- anova(cca_model, by = 'axis')
evals_sig <- summary(cca_model)$cont$importance
percent_sig <- round(evals_sig[2, 1:6] * 100, 2)

# 4. Test par variable : quelles variables contribuent significativement ?
set.seed(1234)
anova_term_cca <- anova(cca_model, by = 'term')
terms_sig <- rownames(anova_term_cca)[anova_term_cca$"Pr(>F)" <= 0.05]
terms_sig <- terms_sig[!is.na(terms_sig)]
```

Le modèle canonique de correspondances a été construit à partir de l'ensemble des variables explicatives retenues :

```{r}
#| echo: false
#| results: asis

format_var_latex <- function(v) {
  m <- regmatches(v, regexec("^([A-Za-z]+)([0-9]*)$", v))[[1]]
  if (length(m) == 3 && nzchar(m[3])) {
    paste0("\\text{", m[2], "}_{", m[3], "}")
  } else {
    paste0("\\text{", v, "}")
  }
}

vars <- attr(terms(cca_model), "term.labels")
form_latex <- paste(sapply(vars, format_var_latex), collapse = " + ")

cat(paste0("$$Y_{\\text{amphibiens}} \\sim ", form_latex, "$$"))
```

#### Significativité du modèle
Ce modèle explique `r round((cca_model$CCA$tot.chi / cca_model$tot.chi) * 100, 1)`% de la variance totale de la composition en amphibiens. Parmi cette part expliquée, nos variables environnementales rendent compte de `r round((cca_model$CCA$eig[1] / cca_model$CCA$tot.chi) * 100, 1)`% de la variance sur le premier axe (CCA1) et de `r round((cca_model$CCA$eig[2] / cca_model$CCA$tot.chi) * 100, 1)`% sur le deuxième (CCA2).

Afin de valider la robustesse de ce modèle, un test ANOVA par permutation (999 permutations) a d'abord été réalisé pour évaluer si l'inertie contrainte par l'ensemble des variables était globalement significative. Ce test révèle que nos variables explicatives influencent significativement la composition en amphibiens (F = `r round(anova_cca$F[1], 2)`, p = `r anova_cca$"Pr(>F)"[1]`).

Ce résultat global ne précisant pas *quelles* variables sont réellement responsables de cet effet, un second test ANOVA a été réalisé, cette fois variable par variable (`by = 'term'`). Il montre que `r length(terms_sig)` variables sur l'ensemble testé sont significatives (p <= 0,05) : `r paste(terms_sig, collapse = " et ")`. Ce sont donc ces variables qui seront retenues pour l'interprétation biologique détaillée ci-après.

Enfin, un troisième test (`by = 'axis'`) a permis de vérifier la significativité de chaque axe d'ordination pris individuellement. Les six premiers axes ressortent tous comme significatifs (p < 0,05). Toutefois, dans un choix de parcimonie, seuls les deux premiers axes ont été retenus pour l'interprétation, ceux-ci concentrant à eux seuls la majorité de la variation expliquée par les variables environnementales, soit `r sum(percent_sig[1:2])`% au total (@fig-cca-interties).

````{r}
#| label: fig-cca-interties
#| fig-cap: "Histogramme des valeurs propres representants les inerties des premiers axes étudiés"
#| echo: false
#| fig-width: 7
#| fig-height: 4
inertie_pourcentage <- (cca_model$CA$eig / sum(cca_model$CA$eig)) * 100
barplot(inertie_pourcentage[1:10], 
        main = "Inertie des axes CCA", 
        ylab = "% d'inertie", 
        xlab = "Axes",
        col = "steelblue")
````
...
Sur la base de ces deux axes retenus, nous avons représenté graphiquement les espèces et les variables explicatives significatives afin d'en interpréter les associations écologiques (@fig-cca-graph-final). Cette analyse reste descriptive et relative à la position des points d'ordination 💡^[ Les variables catégoriques sont interprétées à partir de la distance entre le centroïde de chaque sous-catégorie et celui des espèces : plus les deux centroïdes sont proches, plus l'association écologique supposée entre l'espèce et cette modalité est forte. Les variables numériques, en revanche, sont représentées par un vecteur et doivent être interprétées comme un gradient : la position d'une espèce le long de ce vecteur indique son association aux valeurs faibles ou élevées de la variable, indépendamment de toute notion de distance.]. Pour une confirmation statistique de ces associations, voir la @sec-glm.

```{r}
#| label: fig-cca-graph-final
#| fig-cap: "Graphiques d'ordination de la CCA (espèces et variables) sur les premiers axes."
#| echo: false
#| fig-width: 10
#| fig-height: 4

{
  ...
  # 1. Calcul des limites unifiées
  factor_terms      <- terms_sig[sapply(terms_sig, function(v) is.factor(data_var_cca[[v]]))]
  continuous_terms  <- setdiff(terms_sig, factor_terms)
  bp_scores <- scores(cca_model, display = "bp", scaling = 2)
  cn_scores <- scores(cca_model, display = "cn", scaling = 2)
  bp_sel <- bp_scores[rownames(bp_scores) %in% continuous_terms, , drop = FALSE]
  cn_sel <- cn_scores[sapply(rownames(cn_scores), function(rn) any(startsWith(rn, factor_terms))), , drop = FALSE]
  lim <- max(abs(rbind(bp_sel, cn_sel))) * 1.2
  
  # 2. Panneau Espèces
  plot(cca_model, display = "species", scaling = 2, main = "Espèces",
       xlim = c(-lim, lim), ylim = c(-lim, lim))
  
  # 3. Panneau Variables
  plot(cca_model, type = "n", scaling = 2, main = "Variables",
       xlim = c(-lim, lim), ylim = c(-lim, lim))
  abline(h = 0, v = 0, col = "grey80", lty = 2)
  arrows(0, 0, bp_sel[, 1], bp_sel[, 2], length = 0.1, col = "red", lwd = 2)
  text(bp_sel[, , drop = FALSE], labels = rownames(bp_sel), col = "red", pos = 4, font = 2)
  points(cn_sel[, , drop = FALSE], pch = 17, col = "blue")
  text(cn_sel[, , drop = FALSE], labels = rownames(cn_sel), col = "blue", pos = 4, cex = 0.8)
}
```


Concernant la variable catégorique $TR$ (type de réservoir d'eau) :

* `TR7` → associée au *Triton crêté*
* `TR15` → associée aux *Brown.frogs* et au *Common.toad*
* `TR2` → aucune espèce associée, trop éloignée des centroïdes
* `TR1`, `TR5`, `TR11` → aucune association discutée, sous-catégories très proches de l'origine (contribution négligeable au modèle), donc leur position n'est pas interprétable de façon fiable

Concernant la variable numérique $OR$ (pourcentage d'accès des réservoirs aux aires non développées) :

* Valeurs élevées de OR → associées au *Triton crêté*, dont la projection sur ce gradient est nettement la plus forte
* Projections positives mais modérées, traduisant une association plus faible aux valeurs élevées de OR → *Tree.frog*, *Fire.bellied.toad*, *Common.newt*
* Projection proche de zéro → *Brown.frogs*, peu associée à ce gradient
* Projections négatives, associées aux valeurs basses de OR → *Common.toad*, *Green.frogs* (cette dernière étant l'espèce la plus fortement associée à ce pôle)

#### Conclusion CCA
En conclusion....

### Modele linéaire généralisé {#sec-glm}
...
Ce chapitre consiste à explorer davantage les liens entre les variables environnementales et l'occurence des espèces d'amphibiens. Plus précisément, il tente de répondre à la question : Quels facteurs influencent l'occurrence des amphibiens au sein des routes ? Pour y répondre, des modèles GLM seront utilisés pour chaque espèce afin de voir si les variables concernées change en fonction de celle-ci. 

##### Colinéarité 
Comme démontré précédemment, les paires RR-BR, UR-FR et MR-CR sont associées à des coefficients de colinéarité supérieurs à 0,6. Toutes ces paires sont significativement colinéaires. De même que pour les précédentes analyses, les variables BR, FR et MR sont retirées. 
...
cor.test(data$MR, data$CR, method = "spearman") # pour des raisons d'interprétation, on supprime MR
...
Préalablement à l'analyse GLM des occurences de chaque espèce, une visualisation des ratios de présence/absence selon les routes a été effectué. Les espèces d'amphibiens présentent des ratios très variables entre eux. Les espèces de tritons sont très peu observés comparativement aux autres espèces.  Entre les routes, les ratios différent légèrement avec systématiquement plus de présence au sein de S52.

```{r chapitre4-mosaic, echo = FALSE, warning = FALSE, message = FALSE}

par(mfrow = c(2, 4), mar = c(4, 4, 3, 1))
mosaicplot(
  table(data$Motorway, data$`Green.frogs`),
  main = "Grenouille verte",
  xlab = "Autoroute", ylab = "Occurrence", color = TRUE
)
mosaicplot(
  table(data$Motorway, data$`Brown.frogs`),
  main = "Grenouille rousse",
  xlab = "Autoroute", ylab = "Occurrence", color = TRUE
)
mosaicplot(
...
mosaicplot(
  ...
  main = "Crapaud sonneur",
  xlab = "Autoroute", ylab = "Occurrence", color = TRUE
)
mosaicplot(
  table(data$Motorway, data$`Tree.frog`),
  main = "Grenouille arboricole",
  xlab = "Autoroute", ylab = "Occurrence", color = TRUE
)
mosaicplot(
...
##### Analyse GLM pour chaque espèce 

```{r chapitre4-mod, echo = FALSE, warning = FALSE, message = FALSE}
### Modèle Grenouille verte 
modele_GF <- glm(
  Green.frogs ~  SR+NR+TR+VR+SUR1+SUR2+SUR3+UR+OR+RR+CR, 
  data = dataClean, 
  family = binomial
)
### Modèle Grenouille rousse 
modele_BF <- glm(
  Brown.frogs ~  SR+NR+TR+VR+SUR1+SUR2+SUR3+UR+OR+RR+CR, 
  data = dataClean, 
  family = binomial
)
### Modèle Crapaud commun
modele_CT <- glm(
  Common.toad ~  SR+NR+TR+VR+SUR1+SUR2+SUR3+UR+OR+RR+CR, 
  data = dataClean, 
  family = binomial
)
### Modèle Crapaud sonneur
modele_FBT <- glm(
  Fire.bellied.toad ~  SR+NR+TR+VR+SUR1+SUR2+SUR3+UR+OR+RR+CR, 
  data = dataClean, 
  family = binomial
)
### Modèle Grenouille arboricole
modele_TF <- glm(
  Tree.frog ~  SR+NR+TR+VR+SUR1+SUR2+SUR3+UR+OR+RR+CR, 
  data = dataClean, 
  family = binomial
)
### Modèle Triton commun
modele_CN <- glm(
  Common.newt ~  SR+NR+TR+VR+SUR1+SUR2+SUR3+UR+OR+RR+CR, 
  data = dataClean, 
  family = binomial
)
### Modèle Triton crêté
modele_GCT <- glm(
  Great.crested.newt ~  SR+NR+TR+VR+SUR1+SUR2+SUR3+UR+OR+RR+CR, 
  data = dataClean, 
  family = binomial
)

# Regroupement des modèles 
modeles_especes <- list(
  "Grenouille verte"      = modele_GF,
  "Grenouille rousse"     = modele_BF,
  "Crapaud commun"        = modele_CT,
  "Crapaud sonneur"       = modele_FBT,
  "Grenouille arboricole" = modele_TF,
  "Triton commun"         = modele_CN,
  "Triton crêté"          = modele_GCT
)
...
```
Comme anticipé de par la très faible occurence de certaines espèces et le nombre élevé de variables (13), certains modèles présentent des erreurs types anormalement élevées (>700). Les p-values associés à ces trois modèles ne sont donc pas interprétables. 